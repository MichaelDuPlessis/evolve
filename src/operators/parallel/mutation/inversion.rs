...
/// Parallel version of [`Inversion`](crate::operators::sequential::mutation::Inversion).
///
/// Distributes individuals across pool workers for inversion mutation.
...
impl<T> Inversion<T> {
    /// Creates a new parallel `Inversion` mutation operator.
    pub fn new() -> Self {
        Self {
            _marker: PhantomData,
        }
...
impl<T> Default for Inversion<T> {
    fn default() -> Self {
        Self::new()
    }
}

impl<G, F, R, Fe, T, C> GeneticOperator<G, F, Fe, R, C> for Inversion<T>
where
